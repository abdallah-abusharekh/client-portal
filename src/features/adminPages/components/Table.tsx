...
export default function Table({
  ...
  return (
    <div className="border border-gray-200 rounded-lg w-full overflow-x-auto">
      <table className="rounded-lg min-w-[760px] w-full">
        <thead className="bg-gray-100 border-gray-200 border-b-2">
          <tr>
            {columnKeys.map((key) => (
              <th
                key={key}
                className="p-3 font-semibold text-sm text-start capitalize tracking-wide whitespace-nowrap"
              >
            ...
            {showActions && (
              <th className="p-3 font-semibold text-sm text-start tracking-wide whitespace-nowrap">
                Actions
              </th>
            )}
          </tr>
          ...
          {items.map((item) => (
            <tr key={item.id} className="even:bg-gray-50 odd:bg-white">
              {columnKeys.map((key) => {
                ...
                return (
                  <td key={key} className="p-3 border-gray-700 text-sm whitespace-nowrap">
                    {key === "name" ? (
                      <div className="flex items-center gap-2">
                        {showRowSelection && (
                          <input
              ...
              {showActions && isUserRow(item) && (
                <td className="p-3 border-gray-700 text-sm whitespace-nowrap">
